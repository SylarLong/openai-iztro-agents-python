...
custom ``iztro_tools`` field on the chat-completion response. The base SDK drops every
...
* streaming → an :class:`IztroToolsStreamEvent` is emitted as each tool is called.
...
class IztroToolsStreamEvent:
    ...
    ``.tools`` is the new labels for this batch; ``.type`` is the literal ``"iztro_tools"``.
    """
    ...
    def __repr__(self) -> str:  # pragma: no cover - debugging aid
        return f"IztroToolsStreamEvent(tools={self.tools!r})"
...
class IztroZiweiModel(OpenAIChatCompletionsModel):
    ...
    * Streaming (``Runner.run_streamed``): an :class:`IztroToolsStreamEvent` is emitted
      in ``stream_events()`` as each tool is called.

    ``model.last_iztro_tools`` is a convenience holding the most recent call's tools.
    """
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        # to emit IztroToolsStreamEvent at the right point in the event stream.
    ...
    async def stream_response(self, *args: Any, **kwargs: Any):  # type: ignore[override]
        # Wrap the base streamed events and splice in an IztroToolsStreamEvent as each
        ...
        async for event in super().stream_response(*args, **kwargs):
            while self._stream_pending:
                yield IztroToolsStreamEvent(self._stream_pending.pop(0))
            yield event
        while self._stream_pending:
            yield IztroToolsStreamEvent(self._stream_pending.pop(0))
    ...
    async def _capture_stream(self, raw_stream: Any):
        ...
        # Accumulate + dedup: queue each NEW tool for an IztroToolsStreamEvent, keep the
        ...
        async for chunk in raw_stream:
            ...
            if new:
                ...
                self._stream_pending.append(list(new))  # → IztroToolsStreamEvent in stream_response
            yield chunk
...
def iztro_ziwei_model(
    *,
    ...
    run, read the server-side iztro tools from ``result.raw_responses[i].iztro_tools``
    (non-streaming) or from :class:`IztroToolsStreamEvent` (streaming).
